...
import { leaveGame } from '../../../actions/gameActions/gameActions';
...
function WinnerModal({ winner, leaveGame }) {
  ...
  const losersList = loserPlayers.map(player => <p className={classes.loserStyle}>{player}, </p>);

  const flag = winner === 'britain' ? britainFlag : winner === 'france' ? franceFlag : dutchFlag;
    ...
    <div className={classes.container}>
      <Paper className={classes.paperStyle}>
        ...
        <p className={classes.winnerContainer}>
          The Winner Team is: {winnerTeam} <img src={flag} alt="" className={classes.iconStyle} />
        </p>{' '}
        <p>Winners: {winnersList}</p>
        <p>Losers: {losersList}</p>
        <Button className={classes.btn} variant="contained" color="primary" onClick={leaveGame}>
          Back to Menu
        </Button>
      </Paper>
    </div>
...
const mapStateToProps = state => {
  return {
    winner: state.game.data.gameStatus.winner,
  };
};
